...
const GST_STATE_CODES: Record<string, string> = {
  ...
  '36': 'Telangana',
  const isAdminOrSupport = user?.role === 'admin' || user?.role === 'super_admin' || user?.role === 'support_agent' || user?.role === 'support_manager';
  const isOwner = defaultProfile?.userId === user?.id;
  const isUnclaimedProfile = !defaultProfile?.userId;
  // Admin/support should not edit from user app; restrict to owner/unclaimed only
  const canEdit = (!isAdminOrSupport && (isOwner || isUnclaimedProfile)) || (!isAdminOrSupport && !defaultProfile);
  '38': 'Ladakh',
  '97': 'Other Territory',
...
export default function Account() {
  ...
  const [website, setWebsite] = useState("");
                      Editing is only available to the account owner in the user app. Admins should manage business profiles in the admin panel.
  ...
  const markBusinessProfileStep = async () => {
    try {
      console.log("👉 Calling setup update API with stepKey: businessProfile");
      const response = await apiRequest("POST", "/api/user/setup/update", { stepKey: "businessProfile" });
      console.log("✅ Setup update response:", response);
  ...
  const updateProfileMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      console.log("🎯 Calling markBusinessProfileStep...");
      await markBusinessProfileStep();
      console.log("🎯 markBusinessProfileStep completed");
    },
    onError: (error: any) => {
