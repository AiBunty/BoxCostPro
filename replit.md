...
*   **Authentication**: Dual authentication support - Google OAuth (via Replit Auth) and email-based signup/login with bcrypt password hashing. Email signup collects Name, Company Name, Mobile, Email, and Password. Email verification is auto-approved in development mode but requires explicit verification in production.
